...
namespace hcle
{
   namespace games
   {
      ...
      public:
         ...
         {
            // --- Penalties ---
            float health_change = get_health(current_ram_ptr_) - get_health(previous_ram_.data());
            float damage_penalty = std::min(health_change, 0.0f); // Negative reward for taking damage

            // --- Rewards ---
            float rupee_reward = static_cast<float>(changeIn(RUPEES));
            float key_reward = static_cast<float>(changeIn(KEYS)) * 5.0f;   // Keys are valuable
            float bomb_reward = static_cast<float>(changeIn(BOMBS)) * 0.5f; // Bombs are less valuable
            float combat_reward = static_cast<float>(changeIn(KILLED_ENEMY_COUNT)) * 2.0f;
            float exploration_reward = (current_ram_ptr_[MAP_LOCATION] != previous_ram_[MAP_LOCATION]) ? 10.0f : 0.0f;
            float major_item_reward = static_cast<float>(check_new_items()) * 50.0f; // Big reward for major items

            // Combine all reward components
            float total_reward = damage_penalty +
                                 rupee_reward +
                                 key_reward +
                                 bomb_reward +
                                 combat_reward +

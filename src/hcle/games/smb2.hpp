    ...
    namespace games
    {
        ...
        public:
            ...
            float getReward() override
            {
                ...
                float progress_reward = static_cast<float>(current_progress - previous_progress);

                float damage_penalty = (current_ram_ptr_[PLAYER_HEALTH] < previous_ram_[PLAYER_HEALTH]) ? -25.0f : 0.0f;
                float finish_bonus = (current_ram_ptr_[LEVEL_TRANSITION] == 0x03) ? 100.0f : 0.0f;

                return std::max(progress_reward, 0.0f) + damage_penalty + finish_bonus;
            }

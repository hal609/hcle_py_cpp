...
static const std::map<std::string, std::string> EXPECTED_ROM_HASHES = {
    ...
    {"mariobro.bin", "d85e4dbfb52687c83915ac3e4cc08bbb"},
...
namespace hcle
{
   ...
   inline std::string get_rom_path(const std::string &game_name)
   {
      fs::path roms_dir;
      ...
      if (env_path)
      {
         roms_dir = fs::path(env_path);
         std::cout << "Loading roms from " << fs::absolute(roms_dir) << "..." << std::endl;
      }
      else
      {
         ...
         // !!! TEMP -> CHANGE BEFORE RELEASE !!!
         roms_dir = "/Users/hal/Documents/Programming Projects/cpp_project/hcle_py_cpp/src/hcle/python/hcle_py/roms/";
      }

      // Check dir exists
      if (!fs::exists(roms_dir))
      {
         throw std::runtime_error("ROM directory does not exist: " + roms_dir.string());
      }
      if (!fs::is_directory(roms_dir))
      {
         throw std::runtime_error("ROM path is not a directory: " + roms_dir.string());
      }

      std::string bin_file = game_name + ".bin";
      fs::path bin_path = roms_dir / bin_file;

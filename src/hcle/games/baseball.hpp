   ...
   namespace games
   {
      ...
      public:
         ...
         float getReward() override
         {
            ...
            reward += score_change * 500.0f;
            reward -= opp_score_change * 500.0f;
            ...
            if (is_batting())
            {
               reward += get_bases() * 100.0f;
               reward -= balls_change;
               reward -= outs_change_from_strikes * 10.0f;
               reward -= strikes_change_from_outs * 100.0f;
            }
            else // Pitching
            {
               reward += balls_change;
